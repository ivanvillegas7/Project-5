	...
	while (t <= (my_system.T_ + my_system.dt_)) {
		...
		for (int i = 0; i < my_system.M_ - 2; i++) {
			...
			for (int j = 0; j < my_system.M_ - 2; j++) {
				...
				U(i, j) = imag(u(k));  //We transform the vector u into a matrix U to be able to store it in a cube with the rest time steps (only the imaginary part in this case)

			}

		}

		u_n.slice(l) = U;  //We store the imaginary part of each time step before evolving the system because we want to store also the initial state
